...
Ну так вот, в итоге получается очень много `case`-выражений (далее кейсов). Мне
вручную их писать не хотелось, значит надо это дело передать кому-нибудь.
...
`m4` — доисторический макропроцессор, развитие макропроцессор `m3` (ожидаемо,
...
макросами первой итерации, и потом их редактировал вручную. Понял, что так
нельзя. Делать ещё больше непонятных макросов `m4` не хотел, поэтому решил
...
- Если это был массив вообще, то его элементы конкатились, и вот это вот и
  вставлялось в код на си прямо так.

Проблема: не совсем понятно что чему соответствует.
...
течение дня (а для прошлых итерация я корячился заметное время).
...
неэргономичностью некоторых кнопок. Но это мелочь, конечно же.
